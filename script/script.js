...
(() => {
     ...
     * @type {number}
     */
    const BACKGROUND_STAR_MAX_SIZE = 3 * (3);
    /**
     * 背景を流れる星の最大速度
     * @type {number}
     */
    ...
    function initialize(){
        ...
        for(i = 0; i < SHOT_MAX_COUNT; ++i){
            ...
            shotArray[i] = new Shot(ctx, 0, 0, 32, 32, './image/viper_shot_2.png');
        ...
        viper = new Viper(ctx, 0, 0, 64, 64, './image/viper_4.png');
        ...
        for(i = 0; i < ENEMY_SHOT_MAX_COUNT; ++i){
            ...
            enemyShotArray[i] = new Shot(ctx, 0, 0, 32, 32, './image/enemy_shot_2.png');
        ...
        for(i = 0; i < HOMING_MAX_COUNT; ++i){
            ...
            homingArray[i] = new Homing(ctx, 0, 0, 32, 32, './image/enemy_shot_3.png');
        ...
        for(i = 0; i < ENEMY_SMALL_MAX_COUNT; ++i){
            ...
            enemyArray[i] = Math.random(0, 2) == 0 ? new Enemy(ctx, 0, 0, 48, 48, './image/enemy_small_2.png') : new Enemy(ctx, 0, 0, 48, 48, './image/enemy_small_4.png');
            // 敵キャラクターはすべて同じショットを共有するのでここで与えておく
        ...
        for(i = 0; i < ENEMY_LARGE_MAX_COUNT; ++i){
            ...
            enemyArray[ENEMY_SMALL_MAX_COUNT + i] = Math.random(0, 2) == 0 ? new Enemy(ctx, 0, 0, 64, 64, './image/enemy_large_2.png') : new Enemy(ctx, 0, 0, 64, 64, './image/enemy_large_4.png');
            // 敵キャラクターはすべて同じショットを共有するのでここで与えておく
    ...
    function sceneSetting(){
        ...
        scene.add('gameover', (time) => {
            ...
            if(restart === true){
                ...
                restart = false;
                // スコアをリセットしておく
                document.getElementById('score_log').textContent = "[★[☆[ score_log ]☆]★]";
                const endScore = document.createElement('p');
                endScore.textContent = gameScore;
                document.getElementById('scored').prepend(endScore);
                gameScore = 0;
                // 再度スタートするための座標等の設定
                viper.setComing(
                    CANVAS_WIDTH / 2,   // 登場演出時の開始 X 座標
    ...
    function render(){
        ...
        util.drawText("Shot : Z", 440, 50, '#ffffff');
        util.drawText("Move : ↑↓←→", 440, 80, '#ffffff');
